...
import { ArrowLeftIcon, MessageIcon } from "@/components/ui/icons";
...
export default function ActivateAccountPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      router.push(`/otp?phone=${encodeURIComponent(normalized)}&next=set-password${devOtpParam}`);
    } catch {
  ...
  return (
    <main className="flex min-h-screen flex-col items-start bg-[#F6F1F8] px-6 pb-6 pt-11">
      ...
      <p className="flex w-full justify-center gap-1 pb-2 font-body text-[13px]">
        <span className="text-text-secondary">Already activated?</span>
