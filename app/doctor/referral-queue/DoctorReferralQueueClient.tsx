...
const FILTERS = ["All", "Critical", "High", "Medium", "Awaiting", "Accepted"] as const;
...
export default function DoctorReferralQueueClient({ referrals }: { referrals: DoctorReferralQueueItem[] }) {
  ...
  const filtered = referrals.filter((r) => {
    ...
    if (filter === "Awaiting") return r.status === "SENT";
    if (filter === "Accepted") return r.status === "PATIENT_ARRIVED";
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="font-body text-xs text-text-secondary">
                      {r.week != null ? `Wk ${r.week} · ` : ""}Age {r.age}
                    </p>
                  </div>
                  <PriorityBadge priority={r.priority} className="px-2.5 py-1 text-xs" />
                </div>
                <p className="mt-1.5 font-body text-xs text-text-secondary">← {r.fromFacilityName}</p>
          ...
          <table className="mt-5 w-full">
            <thead>
              <tr className="border-b border-border-color text-left">
                <th className="px-6 py-3 font-body text-xs font-medium text-text-secondary">Patient</th>
                ...
                <th className="px-3 py-3 font-body text-xs font-medium text-text-secondary">Received</th>
            ...
            <tbody>
              ...
              {filtered.map((r) => {
                const status = DOCTOR_REFERRAL_STATUS[r.status];
                const showAction = actionRowId === r.id && hasNextAction(r.status);
                return (
                  <tr key={r.id} className="border-b border-border-color last:border-b-0">
                    <td className="whitespace-nowrap px-6 py-3.5">
                      <p className="font-heading text-sm font-bold text-text-primary">{r.patientName}</p>
                      <p className="font-body text-xs text-text-secondary">
                        {r.week != null ? `Wk ${r.week} · ` : ""}Age {r.age}
                      </p>
                    </td>
                    ...
                    <td className="px-4 py-3.5 text-right">
                      {hasNextAction(r.status) && (
                        <button
                          type="button"
                          onClick={() => setActionRowId((prev) => (prev === r.id ? null : r.id))}
                          aria-label="Toggle actions"
                        >
                          <ChevronRightIcon
                            className={cn("size-3.5 text-[#9CA3AF] transition-transform", showAction && "rotate-90")}
                          />
                        </button>
                      )}
                    </td>
