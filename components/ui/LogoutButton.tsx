...
export default function LogoutButton({ variant = "badge" }: { variant?: "badge" | "row" | "card" | "menu-item" }) {
  ...
  if (variant === "menu-item") {
    return (
      <button
        ...
        onClick={handleLogout}
        disabled={loading}
        className="flex w-full items-center px-3.5 py-2 text-left font-body text-sm font-medium text-critical hover:bg-[#F8FAFC] disabled:opacity-60"
      >
        {loading ? "Signing out…" : "Sign out"}
      </button>
    );
